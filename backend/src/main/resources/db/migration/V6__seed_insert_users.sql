...
SELECT
    gen_random_uuid(), -- ID OBLIGATORIO
    'admin',
...
SELECT
    gen_random_uuid(), -- ID OBLIGATORIO
    'Maximiliano', 'Echeverria', 'DNI', '40123456', '20401234567', n.id, 'SINGLE', 'MALE', DATE '1997-08-15', c.id, 'ejemploprueba1112@gmail.com', '+54 11 5555-1111', '+54 11 4444-2222', 'Av. Libertador', '1234', '5', 'A', ci.id, p.id, '1001', 'ACTIVE', NULL, u.id, 0
FROM users u
         JOIN countries n ON n.iso_code = 'AR'
         JOIN countries c ON c.iso_code = 'AR'
         JOIN provinces p ON p.name = 'Buenos Aires'
         JOIN cities ci ON ci.name = 'La Plata'
WHERE u.email = 'ejemploprueba1112@gmail.com'
    ON CONFLICT (user_id) DO NOTHING;

-- 3. CREW MEMBERS (Aquí NO agregues gen_random_uuid)
-- Porque crew_members hereda el ID de people (es una relación 1 a 1)
INSERT INTO crew_members (id, file_number, maritime_book_number, navigation_role, category, hire_date, current_status, version)
SELECT
    pe.id, -- Este ID ya viene de la tabla people
    'LG00001', 'MB-AR-00001', 'ADMIN', 'SENIOR', CURRENT_DATE, 'AVAILABLE', 0
FROM people pe WHERE pe.email = 'ejemploprueba1112@gmail.com'
